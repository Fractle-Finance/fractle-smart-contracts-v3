...
library MarketMathCore {
        ...
        returns (int256 netSyToAccount, int256 netSyFee, int256 netSyToReserve)
    {
        ...
        /// CHECKS
        /// ------------------------------------------------------------
        if (MiniHelpers.isExpired(market.lifeCircle, blockTime))
            revert Errors.MarketExpired();
    ...
    function _getRateAnchor(
        ...
        int256 newExchangeRate = _getFPTExchangeRateFromImpliedRate(
            lastLnImpliedRate,
            sAPR,
            timeToExpiry
        );
    ...
    function _getLnImpliedRate(
        ...
        for (
            uint256 iter = 0;
            iter < guessNewImpliedRate.maxIteration;
            ++iter
        ) {
            ...
            if (exchangeRateFromImpliedRate.Uint() >= exchangeRate.Uint()) {
                if (
                    PMath.isAGreaterApproxB(
                        exchangeRateFromImpliedRate.Uint(),
                        exchangeRate.Uint(),
                        guessNewImpliedRate.eps
                    )
                ) return guess;
    ...
    function _getFPTExchangeRateFromImpliedRate(
        ...
        int256 sumSequence = (PMath.IONE - xn).divDown(PMath.IONE - x).mulDown(
            x
        ); //x(1-xn)/(1-x),1e18 decimal
    ...
    function setInitialLnImpliedRate(
        ...
        /// CHECKS
        /// ------------------------------------------------------------
        if (MiniHelpers.isExpired(market.expiry, block.timestamp))
            revert Errors.MarketExpired();
