...
contract ActionSwapPT is IPActionSwapPT, ActionBaseMintRedeem {
    ...
    function swapExactPtForSy(
        ...
        (netSyOut, netSyFee) = IPMarket(market).swapExactPtForSy(
            receiver,
            exactPtIn,
            guessNewImpliedRate,
            EMPTY_BYTES
    ...
    function swapPtForExactSy(
        ...
        if (netPtIn > maxPtIn)
            revert Errors.RouterExceededLimitPtIn(netPtIn, maxPtIn);
    ...
    function swapSyForExactPt(
        ...
        if (netSyIn > maxSyIn)
            revert Errors.RouterExceededLimitSyIn(netSyIn, maxSyIn);
    ...
    function swapExactTokenForPt(
        ...
        uint256 netSyUseToBuyPt = _mintSyFromToken(
            address(market),
            address(SY),
            1,
            input
