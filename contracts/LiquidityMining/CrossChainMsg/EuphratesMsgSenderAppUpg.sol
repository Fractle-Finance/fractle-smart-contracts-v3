...
abstract contract EuphratesMsgSenderAppUpg is BoringOwnableUpgradeable {
    using EnumerableMap for EnumerableMap.UintToAddressMap;

    uint256 public approxDstExecutionGas;

    IPMsgSendEndpoint public immutable euphratesMsgSendEndpoint;
    ...
    constructor(address _euphratesMsgSendEndpoint, uint256 _approxDstExecutionGas) {
        euphratesMsgSendEndpoint = IPMsgSendEndpoint(_euphratesMsgSendEndpoint);
    ...
    function _sendMessage(uint256 chainId, bytes memory message) internal {
        ...
        uint256 fee = euphratesMsgSendEndpoint.calcFee(toAddr, chainId, message, estimatedGasAmount);
        ...
        euphratesMsgSendEndpoint.sendMessage{ value: fee }(
            toAddr,
            chainId,
            message,
            estimatedGasAmount
    ...
    function _getSendMessageFee(
        ...
        return
            euphratesMsgSendEndpoint.calcFee(
